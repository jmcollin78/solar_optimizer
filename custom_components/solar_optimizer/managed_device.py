...
class ManagedDevice:
    ...
    def should_be_forced_offpeak(self) -> bool:
        ...
        if self._offpeak_time >= self._coordinator.raz_time:
            return (self.now.time() >= self._offpeak_time or self.now.time() < self._coordinator.raz_time) and self._on_time_sec < self.min_on_time_per_day_sec and self._on_time_sec < self.max_on_time_per_day_sec
        else:
            return self.now.time() >= self._offpeak_time and self.now.time() < self._coordinator.raz_time and self._on_time_sec < self.min_on_time_per_day_sec and self._on_time_sec < self.max_on_time_per_day_sec
